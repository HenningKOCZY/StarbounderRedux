...
//	var world : int = gameMasterScript.worldNum;
	for (var i: int=0; i<rens.length; i++) {
		if(i==0) rens[i].SetColor ("_TintColor", palette[i+(0)]);
		if(i==1) rens[i].SetColor ("_TintColor", palette[i+(0)]);
		if(i==2) rens[i].SetColor ("_TintColor", palette[i+(0)]);
	}
}

function Update () {
	//print(rens[2].mainTextureOffset.x);
	for (var i: int=0; i<rens.length; i++) {
		rens[i].mainTextureOffset = Vector2 (Time.time*-1*uSpeeds[i], Time.time*vSpeeds[i]);
		
		//rens[i].mainTextureOffset.x -= Mathf.Floor(rens[i].mainTextureOffset.x);

...
public class GUI_LevelSelect : MonoBehaviour
{
	...
	public List<Text> ls_artiTexts = new List<Text> ();
	int worldNum;
	int maxLevel;
	int maxWorld;
	int tutorialState;
	...
	void Awake ()
	{
		guinew = transform.parent.GetComponent<GUInew> (); // need gui for world title cards only?
	}
...
//				print ("level hit: " + (worldNum * 10 + digits));
	...
	public void Pop ()
	{
		...
		// put the right title on
		title.sprite = guinew.worldTitles [worldNum * 2];
		...
		if (worldNum != 6) { 
			...
			for (int d = 0; d < 10; d++) {
				...
				int level = (worldNum * 10) + d;
				Text lsbt = ls_buttonTexts [d];
...
//					if ((int)Mathf.Floor (artCount / 100) == 1)
//						artiString += "* ";
//					else
//						artiString += "- ";
//					if ((int)Mathf.Floor ((artCount % 100) / 10) == 1)
//						artiString += "* ";
//					else
//						artiString += "- ";
//					if ((int)artCount % 10 == 1)
//						artiString += "* ";
//					else
//						artiString += "- ";

					Text lsat = ls_artiTexts [d];
					lsat.text = artiString;
		...
		} else { // special button names for tut/cin
